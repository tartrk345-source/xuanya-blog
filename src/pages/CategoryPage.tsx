...
export default function CategoryPage() {
  ...
  if (!catInfo) {
    return (
      <div className="min-h-screen bg-[#FEF3F0] dark:bg-[#1E293B]">
        <Navigation />
  ...
  return (
    <div className="min-h-screen bg-[#FEFAF9] dark:bg-[#0A0E1A] text-[#313131] dark:text-[#E2E8F0] font-['PingFang_SC','Microsoft_YaHei','Noto_Sans_SC',sans-serif] transition-colors duration-300">
      <Helmet>
      ...
      <div className="pt-16 sm:pt-20 pb-8 px-4 sm:px-8">
        <div className="max-w-[1100px] mx-auto">
          ...
          <div className="flex items-center gap-4 mb-4">
            ...
            <div>
              <h1 className="text-[clamp(1.8rem,4vw,2.8rem)] font-extrabold text-[#313131] dark:text-[#E2E8F0] tracking-wider leading-tight font-['PingFang_SC','Noto_Serif_SC',serif]">
                {catInfo.label}
              </h1>
              <p className="text-[1.05rem] text-[#6E6A7C] dark:text-[#94A3B8] mt-2">{catInfo.description}</p>
            </div>
          </div>

          <div className="flex items-center gap-4 pt-2">
            <span className="text-xs text-[#B8B4B0] dark:text-[#94A3B8]">{articles.length} 篇文章</span>
      ...
      {isEmpty ? (
        <div className="max-w-[1100px] mx-auto px-4 sm:px-8 pb-32">
          <div className="text-center py-20 bg-white dark:bg-[#1E293B] border border-[#ECD8D9] dark:border-[#334155] rounded-3xl">
            <div className="text-4xl mb-4">{catInfo.icon}</div>
            <h3 className="text-lg font-bold text-[#313131] dark:text-[#E2E8F0] mb-2">还没有文章</h3>
            <p className="text-sm text-[#767693] dark:text-[#94A3B8]">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
            {articles.map(a => (
              <Link
                key={a.id}
                to={`/article/${a.id}`}
                className="group bg-white dark:bg-[#1E293B] border border-[#ECD8D9] dark:border-[#334155] rounded-2xl p-4 sm:p-6 hover:border-[#DA583F] hover:-translate-y-0.5 hover:shadow-[0_8px_30px_rgba(218,88,63,0.06)] transition-all duration-300 flex flex-col"
              >
                ...
                <p className="text-xs text-[#767693] dark:text-[#94A3B8] line-clamp-2 mb-4 ml-8 flex-1">
