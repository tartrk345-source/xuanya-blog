...
export default function MarkdownEditor({ content, onChange }: MarkdownEditorProps) {
  ...
  return (
    <div className="border border-[#ECD8D9] dark:border-[#334155] rounded-xl overflow-hidden bg-white dark:bg-[#141012] shadow-sm">
      {/* 顶部工具栏 */}
      <div className="flex items-center gap-1 px-3 py-2 border-b border-[#ECD8D9] dark:border-[#334155] bg-[#FDF7F6] dark:bg-[#1E293B] flex-wrap">
        ...
        <div className="w-px h-5 bg-[#ECD8D9] dark:bg-[#334155] mx-1" />
      ...
      <div className="flex">
        ...
        {(!previewOnly) && (
          <div className={`${activeTab === 'split' ? 'w-1/2' : 'w-full'} flex flex-col ${editOnly ? '' : 'md:border-r md:border-[#ECD8D9] dark:md:border-[#2A2020]'}`}>
            ...
            <textarea
              ...
              className="w-full flex-1 min-h-[460px] p-4 resize-none outline-none font-mono text-[14px] leading-[1.8] text-[#313131] dark:text-[#E2E8F0] placeholder-[#B8B4B0] bg-transparent"
            />
          </div>
        )}
        ...
        {(!editOnly) && (
          <div className={`${activeTab === 'split' ? 'w-1/2' : 'w-full'} flex flex-col`} style={{ minHeight: '460px' }}>
            <div className="px-4 py-2 bg-[#FDF7F6] dark:bg-[#1E293B] border-b border-[#ECD8D9] dark:border-[#334155]">
              <span className="text-xs text-[#767693] dark:text-[#94A3B8] font-medium">预览</span>
            </div>
            <div className="flex-1 p-4 overflow-y-auto markdown-preview" style={{ minHeight: '420px' }}>
              {content.trim() ? (
                <ReactMarkdown
                  ...
                  components={{
                    h1: ({ children }) => <h1 className="text-2xl font-bold text-[#313131] dark:text-[#E2E8F0] mb-4 mt-6 first:mt-0">{children}</h1>,
                    ...
                    hr: () => <hr className="border-[#ECD8D9] dark:border-[#334155] my-4" />,
                    table: ({ children }) => <table className="border border-[#ECD8D9] dark:border-[#334155] rounded-lg overflow-hidden my-3 w-full">{children}</table>,
                    thead: ({ children }) => <thead className="bg-[#FDF7F6] dark:bg-[#1E293B]">{children}</thead>,
                    th: ({ children }) => <th className="px-3 py-2 text-left text-sm font-semibold text-[#313131] dark:text-[#E2E8F0] border-b border-[#ECD8D9] dark:border-[#334155]">{children}</th>,
                    td: ({ children }) => <td className="px-3 py-2 text-sm border-t border-[#ECD8D9] dark:border-[#334155] text-[#4F4F4F] dark:text-[#94A3B8]">{children}</td>,
                  }}
                >
                  {content}
                </ReactMarkdown>
      ...
      {/* Markdown 语法参考面板 */}
      {showCheatsheet && (
        <div className="border-t border-[#ECD8D9] dark:border-[#334155] bg-[#FDF9F8] dark:bg-[#110E0F] p-4">
          ...
          <div className="mt-3 p-3 bg-white dark:bg-[#1E293B] rounded-lg border border-[#ECD8D9] dark:border-[#334155]">
            <span className="text-xs text-[#767693] dark:text-[#94A3B8]">表格示例：</span>
            <pre className="mt-1 text-xs text-[#4F4F4F] dark:text-[#94A3B8] font-mono whitespace-pre-wrap">{"| 左对齐 | 居中 | 右对齐 |\n| :--- | :---: | ---: |\n| 内容 | 内容 | 内容 |"}</pre>
          </div>
        </div>
      )}
    </div>
